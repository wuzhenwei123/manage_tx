...
<div class="box">
    ...
    <div class="topBox"><a class=" offen_back" href="javascript:history.go(-1);"></a>我的朋友</div>
            ...
            <div class="itemsContentBox">
                ...
                <div class="searItems">
                    <div class="fl searItem">
                        <input class="input" type="text" placeholder="朋友姓名" id="realName"/>
                    </div>
                    <div class="fr searItem">
                        <input class="input" type="text" placeholder="朋友手机" id="mobile"/>
                    </div>
                </div>
    ...
    <div class="ch_ui_box bsBox marginTop16">
        ...
        <div class="ui_cell ui_cells">
            <div class="ui_center ui_cell_flex jls">朋友赚取钱币总数</div>
            <div class="ui_cell_ft"><font class="" id="poneyPCount"></font></div>
        </div>
        <div class="ui_cell ui_cells">
            <div class="ui_center ui_cell_flex jls">朋友为您赚取钱币总数</div>
            <div class="ui_cell_ft"><font class="" id="moneyCount"></font></div>
        </div>
    </div>
...
<script>
	...
	function loadData(page) {
		...
		$.get("${ctx}/weixin/myFriendSell?pageNo="+page+"&_t="+Math.random()+"&openId=${openId}&state="+state+"&mobile="+mobile+"&realName="+realName,function(data,status){
			var json = eval("("+data+")");
			if(json.message=="ok"){
				$("#fansCount").html(json.totalResults);
				$("#poneyPCount").html(json.poneyPCount);
				$("#moneyCount").html(json.moneyCount);
				$(".loading").remove();
				var testData = json.items
				for(var i in testData) {
					
					var itemTpl = '<div class="bsBox ItemLi marginTop16"><div>';
					itemTpl = itemTpl + '<ul class="clearAfter ItemuL"><li class="fl">朋友手机号</li><li class="fr">{mobile}</li></ul>';
					itemTpl = itemTpl + '<ul class="clearAfter ItemuL"><li class="fl">朋友名称</li><li class="fr">{realName}</li></ul>';
					itemTpl = itemTpl + '<ul class="clearAfter ItemuL"><li class="fl">交易总金额</li><li class="fr">{money}</li></ul>';
					itemTpl = itemTpl + '<ul class="clearAfter ItemuL"><li class="fl">朋友赚取钱币</li><li class="fr">{profit}</li></ul>';
					itemTpl = itemTpl + '<ul class="clearAfter ItemuL"><li class="fl">朋友为您赚取钱币</li><li class="fr">{profit_one}</li></ul>';
