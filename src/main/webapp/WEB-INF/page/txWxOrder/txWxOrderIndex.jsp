													...
													<div class="panel-body">
														<c:if test="${dbrole!=admin_user.role_id&&dbrole2!=admin_user.role_id}">
															<div class="form-group">
																<label class="control-label" for="wxUserId">业务员：</label>
										    		...
										    		<div class="row">
										            	<div class="col-sm-12">
											    			<div id="toolbar">
																...
																<perm:tag permPath="/txWxOrder/exportTxt" >
															    <button type="button" onclick="exportTxt()" class="btn btn-primary">
															        <i class="glyphicon glyphicon-plus"></i>导出分润(TXT)
															    </button>
																</perm:tag>
															</div>
															<table id="table" data-toggle="table"
																data-toolbar="#toolbar"
														        data-url="${ctx}/txWxOrder/getTxWxOrderList"
...
// 					}
	    ...
	    function exportTxt(){
	    	...
	    	if(flag){
	    		flag = false;
	    		$.getJSON("<c:url value='/txWxOrder/exportTxt'/>",
    	        {
	    			startTime:startTime,
	    			endTime:endTime,
    				_t: Math.random()
    	            ...
    	            if (result.message == "ok") {
    	            	...
    	            	location.href = "${ctx}/weixin/downFile?name="+rows.name+"&link="+rows.path; 
    	            } else if(result.message == "nodata"){
    	            	layer.msg("下载失败，您选的日期没有交易记录!");
    	            } else{
